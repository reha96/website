...
**"What is the m dimension here?"** — m is the number of training examples, which this project lays out one per column: X is (nx, m) with nx = number of input features, Y and A are (1, m). (What the /m averaging is for is the −1/m story in my [cross-entropy TIL](/til/ai/cross-entropy-cost-intuition).)

**"Why A = self.forward_prop(X) at every iteration?"** — W and b change every iteration, and A is a function of them — the stale-predictions trap above. Gradient descent is "look at the local slope where you stand, step, look again, step."

**"What about evaluate?"** — Threshold at 0.5: A ≥ 0.5 ⟺ z ≥ 0, so the decision boundary is just "is the weighted sum W·X + b positive?" Cost is computed on the soft A before thresholding, so "barely right" still costs a little — my [cross-entropy TIL](/til/ai/cross-entropy-cost-intuition) has the numbers.
